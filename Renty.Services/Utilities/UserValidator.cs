...
namespace Renty.Services.Utilities
{
    internal static class UserValidator
    {
        ...
        public static void ValidateNickname(string nickname)
        {
            ...
            {
                throw new ArgumentException(string.Format(
                    "DisplayName should be between {0} and {1} symbols long",
                    MinUsernameNicknameChars,
                    MaxUsernameNicknameChars), "nickname");
            }
            else if (nickname.Any(ch => !ValidNicknameChars.Contains(ch)))
            {
                throw new ArgumentException("DisplayName contains invalid characters", "nickname");
            }
        }
